...
int home[17][17], dp[17][17][3], n; // (y, x) with direction dir.. 0: horizontal, 1: diagnoal, 2: vertical
vector<int> r[3];
...
int solve(int sy, int sx, int ey, int ex, int dir)
{
    int &ret = dp[sy][sx][dir];
    if (ey == n && ex == n)
        return ret = 1;
    if (ret != INF)
        return ret;
